...
import { generateTextImage, generateTextVideo, generateStyleSuggestion, generateContentSuggestion, generateTypographySuggestion } from './services/geminiService';
import { getRandomStyle, fileToBase64, TYPOGRAPHY_SUGGESTIONS, createGifFromVideo, STYLE_CATEGORIES, TYPOGRAPHY_CATEGORIES } from './utils';
      ...
      <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-stone-100 dark:border-zinc-800 animate-in zoom-in-95 duration-300">
        <div className="p-6">
          ...
          <p className="text-stone-600 dark:text-stone-300 text-sm leading-relaxed mb-4">
            High-quality video generation using Veo models requires a Google Cloud project with billing enabled. Free-tier API keys will not work.
          </p>

          <div className="bg-stone-50 dark:bg-zinc-800/50 p-4 rounded-xl border border-stone-200 dark:border-zinc-700 mb-6">
            <h3 className="text-sm font-bold text-stone-900 dark:text-white mb-3">How to set this up:</h3>
            <ol className="text-sm text-stone-600 dark:text-stone-400 space-y-3 list-decimal list-inside">
              <li>
                Go to <a href="https://console.cloud.google.com/billing" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline font-medium">Google Cloud Billing</a> and ensure your project has an active billing account.
              </li>
              <li>
                Generate an API key in Google AI Studio linked to that paid project.
              </li>
              <li>
                Click "Select API Key" below and choose your paid key.
              </li>
            </ol>
            <div className="mt-4 pt-3 border-t border-stone-200 dark:border-zinc-700">
              <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noopener noreferrer" className="text-xs text-stone-500 dark:text-stone-400 hover:text-stone-900 dark:hover:text-white flex items-center gap-1 transition-colors w-fit">
                <ExternalLink size={12} /> Read the Gemini API billing documentation
              </a>
            </div>
          </div>
...
const App: React.FC = () => {
  ...
  const [errorType, setErrorType] = useState<'quota' | 'safety' | 'auth' | 'generic' | null>(null);
  ...
  const [isSuggestingTypography, setIsSuggestingTypography] = useState<boolean>(false);
  
  const [contentSuggestions, setContentSuggestions] = useState<string[]>([]);
  const [styleSuggestions, setStyleSuggestions] = useState<string[]>([]);
  const [typographySuggestions, setTypographySuggestions] = useState<string[]>([]);
  ...
  const startProcess = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const videoUrl = await generateTextVideo(inputText, b64Image, mimeType, styleToUse, (msg, prog) => {
        setStatusMessage(msg);
        if (prog !== undefined) setProgress(prog);
      });
      ...
      if (msg.includes("Requested entity was not found") || msg.includes("404")) {
        ...
        let displayMsg = "Something went wrong creating your art.";
        let type: 'quota' | 'safety' | 'auth' | 'generic' = 'generic';

        if (msg.toLowerCase().includes("quota") || msg.includes("429")) {
          displayMsg = "Quota exceeded. Please check your API limits or try again later.";
          type = 'quota';
        } else if (msg.toLowerCase().includes("safety") || msg.toLowerCase().includes("blocked")) {
          displayMsg = "Content blocked by safety filters. Please try a different prompt.";
          type = 'safety';
        } else if (msg.toLowerCase().includes("api key not valid") || msg.includes("403") || msg.includes("401")) {
          displayMsg = "API key is invalid or lacks permissions. Please select a valid key.";
          type = 'auth';
        } else if (msg) {
    ...
    if (state === AppState.ERROR) {
       return (
        <div className="flex flex-col items-center justify-center space-y-6 h-full p-8 text-center animate-in zoom-in-95">
          <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-6 py-4 rounded-xl border border-red-100 dark:border-red-900/30 max-w-md shadow-sm w-full">
            <p className="font-medium">Generation Failed</p>
            <p className="text-sm mt-1 text-red-500 dark:text-red-400">{statusMessage}</p>
            <div className="mt-4 pt-4 border-t border-red-200 dark:border-red-900/50 text-xs text-red-500/80 dark:text-red-400/80 text-left space-y-2">
              <p className="font-semibold text-red-600 dark:text-red-400">Potential causes & solutions:</p>
              <ul className="list-disc pl-4 space-y-1">
                {errorType === 'quota' && (
                  <li><strong>Generation Limits:</strong> You have exceeded your quota. Please check your Google Cloud console billing and quotas, or try again later.</li>
                )}
                {errorType === 'safety' && (
                  <li><strong>Safety Filters:</strong> The prompt or generated content triggered safety filters. Please try adjusting your text or style to be more family-friendly.</li>
                )}
                {errorType === 'auth' && (
                  <li><strong>API Key Validity:</strong> Ensure your selected Google Cloud project has billing enabled and the API key is valid.</li>
                )}
                {errorType === 'generic' && (
                  <>
                    <li><strong>API Key Validity:</strong> Ensure your selected Google Cloud project has billing enabled and the API key is valid.</li>
                    <li><strong>Generation Limits:</strong> You may have exceeded your quota. Check your Google Cloud console.</li>
                    <li><strong>Safety Filters:</strong> The prompt or generated content may have triggered safety filters. Try adjusting your text or style.</li>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full max-w-md">
            <button onClick={reset} className="flex-1 px-8 py-3 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 font-medium rounded-full hover:bg-stone-800 dark:hover:bg-white transition-colors shadow-lg">
              Try Again
            </button>
            <button onClick={() => setShowKeyDialog(true)} className="flex-1 px-8 py-3 bg-white dark:bg-zinc-800 text-stone-900 dark:text-white border border-stone-200 dark:border-zinc-700 font-medium rounded-full hover:bg-stone-50 dark:hover:bg-zinc-700 transition-colors shadow-sm">
              Change API Key
            </button>
          </div>
        </div>
      ...
      return (
        <div className="w-full h-full flex flex-col items-center justify-center p-4 md:p-8 bg-stone-50 dark:bg-zinc-950">
          <div className={`flex items-center gap-3 px-5 py-2 rounded-full mb-6 transition-all duration-500 ${state === AppState.PLAYING ? 'opacity-0 h-0 mb-0 overflow-hidden' : 'bg-white dark:bg-zinc-900 shadow-sm border border-stone-100 dark:border-zinc-800'}`}>
             ...
             <span className="text-sm font-medium text-stone-600 dark:text-stone-300 uppercase tracking-wide">
               {statusMessage} {progress > 0 && progress < 100 && `(${progress}%)`}
             </span>
          </div>
          <div className="relative w-full max-w-6xl aspect-video bg-white dark:bg-zinc-900 rounded-2xl overflow-hidden shadow-2xl ring-1 ring-stone-900/5 dark:ring-white/10 group">
            {(state === AppState.GENERATING_IMAGE || state === AppState.GENERATING_VIDEO) && (
              <div className="absolute top-0 left-0 right-0 h-1.5 bg-stone-100 dark:bg-zinc-800 z-20 overflow-hidden">
                <div 
                  className="h-full bg-stone-900 dark:bg-stone-100 transition-all duration-500 ease-out" 
            ...
            {(state === AppState.GENERATING_IMAGE) && !imageSrc && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-stone-50 dark:bg-zinc-900 space-y-6">
                 ...
                 <p className="text-stone-400 dark:text-stone-500 font-medium animate-pulse text-sm">{statusMessage}</p>
              </div>
            )}
            {imageSrc && !videoSrc && state !== AppState.GENERATING_VIDEO && <img src={imageSrc} alt="Text Visualized" className="w-full h-full object-cover animate-in fade-in duration-1000" />}
            {imageSrc && state === AppState.GENERATING_VIDEO && (
               <div className="absolute inset-0 bg-black overflow-hidden flex items-center justify-center">
                  <style>{`
                    @keyframes preview-loop {
                      0% { opacity: 0; transform: scale(1); filter: blur(12px) brightness(0); }
                      30% { opacity: 1; transform: scale(1.03); filter: blur(0px) brightness(1); }
                      70% { opacity: 1; transform: scale(1.07); filter: blur(0px) brightness(1); }
                      100% { opacity: 0; transform: scale(1.1); filter: blur(12px) brightness(0); }
                    }
                  `}</style>
                  <img 
                    src={imageSrc} 
                    ...
                    style={{ animation: 'preview-loop 4s ease-in-out infinite' }}
                  />
                  <div className="absolute inset-0 bg-black/20 flex flex-col items-center justify-center space-y-6 z-10 transition-all">
                    <div className="bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md p-4 rounded-full shadow-xl flex items-center gap-3">
            ...
            {videoSrc && (
              <div className="relative w-full h-full group">
                <video 
                  ...
                  className={`w-full h-full object-cover animate-in fade-in duration-1000 ${isPreviewMode ? 'cursor-pointer' : ''}`}
                  onTimeUpdate={(e) => {
                    if (isPreviewMode && e.currentTarget.currentTime >= 2) {
                ...
                {isPreviewMode && (
                  <div className="absolute inset-0 bg-black/20 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                    <div className="bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md p-4 rounded-full shadow-xl flex items-center gap-3">
                      <Play className="w-6 h-6 text-stone-900 dark:text-white" fill="currentColor" />
                      <span className="text-sm font-bold text-stone-900 dark:text-white">Play Full Video</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button type="button" onClick={async () => {
            ...
            const [content, style] = await Promise.all([
              ...
              generateStyleSuggestion(inputText || 'Neon Dreams')
            ]);
            
            if (content && content.length > 0) {
              setInputText(content[0]);
              setContentSuggestions(content);
            }
            if (style && style.length > 0) {
              setInputStyle(style[0]);
              setStyleSuggestions(style);
            }
            
            const typography = await generateTypographySuggestion(content?.[0] || inputText, style?.[0] || inputStyle);
            if (typography && typography.length > 0) {
              setTypographyPrompt(typography[0]);
              setTypographySuggestions(typography);
            }
            
            setIsSuggestingContent(false);
            setIsSuggestingStyle(false);
            setIsSuggestingTypography(false);
          }} disabled={isSuggestingContent || isSuggestingStyle || isSuggestingTypography} className="px-4 py-2 bg-stone-100 dark:bg-zinc-800 text-stone-900 dark:text-white text-sm font-bold rounded-xl hover:bg-stone-200 dark:hover:bg-zinc-700 transition-colors flex items-center gap-2 disabled:opacity-50">
            {(isSuggestingContent || isSuggestingStyle || isSuggestingTypography) ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />} Surprise Me
          </button>
        </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <button type="button" onClick={async () => {
                    ...
                    if (suggestion && suggestion.length > 0) {
                      setInputText(suggestion[0]);
                      setContentSuggestions(suggestion);
                    }
                    setIsSuggestingContent(false);
                  }} disabled={isSuggestingContent} className="text-xs font-medium text-stone-500 dark:text-stone-400 hover:text-stone-900 dark:hover:text-stone-200 flex items-center gap-1 transition-colors disabled:opacity-50">
                      {isSuggestingContent ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />} {isSuggestingContent ? 'Thinking...' : 'Suggest'}
                  </button>
                </div>
                ...
                {contentSuggestions.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {contentSuggestions.map((opt, idx) => (
                      <button key={idx} type="button" onClick={() => setInputText(opt)} className={`px-2 py-1 hover:bg-stone-200 dark:hover:bg-zinc-700 text-stone-600 dark:text-stone-300 text-[10px] font-medium rounded-md border border-stone-200 dark:border-zinc-700 transition-colors ${inputText === opt ? 'bg-stone-200 dark:bg-zinc-700' : 'bg-stone-100 dark:bg-zinc-800'}`}>
                        {opt}
                      </button>
                    ))}
                  </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <button type="button" onClick={async () => {
                    ...
                    if (suggestion && suggestion.length > 0) {
                      setInputStyle(suggestion[0]);
                      setStyleSuggestions(suggestion);
                    }
                    setIsSuggestingStyle(false);
                ...
                <select 
                  onChange={(e) => {
                    if (e.target.value) setInputStyle(e.target.value);
                  }}
                  className="w-full bg-stone-50 dark:bg-zinc-900 border border-stone-200 dark:border-zinc-800 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-stone-900 dark:focus:ring-stone-100 transition-all text-stone-900 dark:text-white mb-2"
                >
                  <option value="">Select a style category...</option>
                  {Object.entries(STYLE_CATEGORIES).map(([category, items]) => (
                ...
                {styleSuggestions.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {styleSuggestions.map((opt, idx) => (
                      <button key={idx} type="button" onClick={() => setInputStyle(opt)} className={`px-2 py-1 hover:bg-stone-200 dark:hover:bg-zinc-700 text-stone-600 dark:text-stone-300 text-[10px] font-medium rounded-md border border-stone-200 dark:border-zinc-700 transition-colors text-left ${inputStyle === opt ? 'bg-stone-200 dark:bg-zinc-700' : 'bg-stone-100 dark:bg-zinc-800'}`}>
                        {opt}
                      </button>
                    ))}
                  </div>
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <button type="button" onClick={async () => {
                    setIsSuggestingTypography(true);
                    const suggestion = await generateTypographySuggestion(inputText, inputStyle);
                    if (suggestion && suggestion.length > 0) {
                      setTypographyPrompt(suggestion[0]);
                      setTypographySuggestions(suggestion);
                    }
                    setIsSuggestingTypography(false);
                  }} disabled={isSuggestingTypography} className="text-xs font-medium text-stone-500 dark:text-stone-400 hover:text-stone-900 dark:hover:text-stone-200 flex items-center gap-1 transition-colors disabled:opacity-50">
                      {isSuggestingTypography ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />} {isSuggestingTypography ? 'Thinking...' : 'Suggest'}
                  </button>
                </div>
                <select 
                  onChange={(e) => {
                    if (e.target.value) setTypographyPrompt(e.target.value);
                  }}
                  className="w-full bg-stone-50 dark:bg-zinc-900 border border-stone-200 dark:border-zinc-800 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-stone-900 dark:focus:ring-stone-100 transition-all text-stone-900 dark:text-white mb-2"
                >
                  <option value="">Select a typography category...</option>
                  {Object.entries(TYPOGRAPHY_CATEGORIES).map(([category, items]) => (
                    <optgroup key={category} label={category}>
                      {items.map((item, idx) => (
                        <option key={idx} value={item}>{item}</option>
                      ))}
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {typographySuggestions.length > 0 ? (
                    typographySuggestions.map((opt, idx) => (
                      <button key={idx} type="button" onClick={() => setTypographyPrompt(opt)} className={`px-2 py-1 hover:bg-stone-200 dark:hover:bg-zinc-700 text-stone-600 dark:text-stone-300 text-[10px] font-medium rounded-md border border-stone-200 dark:border-zinc-700 transition-colors text-left ${typographyPrompt === opt ? 'bg-stone-200 dark:bg-zinc-700' : 'bg-stone-100 dark:bg-zinc-800'}`}>
                        {opt}
                      </button>
                    ))
                  ) : (
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                   {referenceImage && (
                    <div className="h-10 w-10 relative rounded overflow-hidden border border-stone-200 dark:border-zinc-700 group flex-shrink-0">
